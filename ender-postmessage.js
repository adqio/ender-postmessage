...
!function (window) {
    ...
    fn.receiveMessage = function( callback, source_origin, delay,win ) {
      if(!win){
        win = window;
      }
    if ( has_postMessage ) {
      ...
      win[ callback ? addEventListener : 'removeEventListener' ]( 'message', rm_callback, FALSE );
      } else {
      win[ callback ? 'attachEvent' : 'detachEvent' ]( 'onmessage', rm_callback );
      }
      ...
      original_hash = win.document.location.hash;

      interval_id = setInterval(function(){
        var hash = win.document.location.hash,
        re = /^#?\d+&/;
        if ( hash !== last_hash && hash !== original_hash && re.test( hash ) ) {
        last_hash = hash;
        if ( original_hash ) {
          win.document.location.hash = original_hash;
        } else {
